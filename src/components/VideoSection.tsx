...
export default function VideoSection() {
          ...
          onStateChange: (event: any) => {
            if (event.data === window.YT.PlayerState.PLAYING) {
              intervalRef.current = window.setInterval(() => {
                ...
                const pct = p.getCurrentTime() / duration;
                setProgress(pct);
                if (pct >= UNLOCK_AT) {
                  setUnlocked(true);
  ...
  return (
    <section className="py-14 max-w-3xl mx-auto px-4 space-y-6">
      ...
      {!unlocked && (
        <div className="space-y-2">
          ...
          <p className="text-center text-[11px] font-mono text-gray-500">
            {progress > 0.05
              ? `${Math.round(progress * 100)}% assistido — quase lá para desbloquear o pack`
      ...
      {unlocked && (
        <div className="bg-orange-600/10 border border-orange-500/30 rounded-sm p-6 text-center space-y-4">
