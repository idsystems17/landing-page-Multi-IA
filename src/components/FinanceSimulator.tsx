...
export default function FinanceSimulator() {
  // Constant fixed values for the static visualization
  const ticketPrice = 47.00;
  const traffic = 500; // Daily visits
  const conversionRate = 1.5; // Conversion percentage

  // Roadmap checklist items (static preview)
  const checklist = [
  ...
  // Calculations based on the static values
        ...
        <div className="lg:col-span-7 bg-[#050510]/40 border border-white/10 rounded-2xl p-6 md:p-8 space-y-8 relative overflow-hidden backdrop-blur-md">
          ...
          <div className="space-y-4">
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-purple-400 bg-purple-500/10 border border-purple-500/20 px-2.5 py-1 rounded">
              Planejador Estratégico de Entrada (Visualização Prévia)
            </span>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                value={ticketPrice}
                disabled
                className="w-full accent-orange-500 bg-[#0a0a1a] rounded-sm appearance-none h-1.5 focus:outline-none opacity-50 cursor-default"
              />
              <div className="flex gap-2 pt-1">
                {[30, 47, 97].map((presetPrice) => (
                  <span
                    key={presetPrice}
                    className={`py-1 px-3 text-[11px] font-mono rounded border cursor-default select-none transition-all ${
                      Math.abs(ticketPrice - presetPrice) < 1
                        ? "bg-orange-600/20 border-orange-500 text-orange-400 font-bold"
                        : "bg-[#0a0a15] border-white/5 text-gray-500"
                    }`}
                  >
                    R$ {presetPrice}
                  </span>
          ...
          <div className="border-t border-white/5 pt-6 space-y-6">
            ...
            <div className="flex gap-2.5 items-start p-4 bg-[#110c05] rounded-xl border border-orange-500/10 text-xs font-sans leading-relaxed text-gray-300 font-light">
              ...
              <p>
                <strong className="text-white font-semibold">A essência do Funil:</strong> Com apenas {salesCountMonthly} compras mensais de {formatBRL(ticketPrice)} usando um produto criado em 1h com orquestração Multi-IA, você consegue colocar no seu bolso livre um salário líquido de <strong className="text-emerald-400 font-bold">{formatBRL(totalMonthlyCalculated)}/mês</strong>.
              </p>
            </div>
        ...
        <div className="lg:col-span-5 bg-[#050510]/40 border border-white/10 rounded-2xl p-6 md:p-8 space-y-6 relative overflow-hidden backdrop-blur-md">
          ...
          <div className="space-y-1.5">
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded">
              Desafio Digital 24h (Plano de Trabalho)
            </span>
            <h3 className="text-2xl md:text-3xl font-black text-white tracking-tight uppercase">
              Desafio Digital 24 Horas
            </h3>
            <p className="text-gray-400 text-xs font-sans font-light leading-relaxed">
              Corte o blá-blá-blá teórico. Siga o roteiro pragmático abaixo para estruturar e publicar o seu primeiro infoproduto de alta escala.
            </p>
          </div>

          {/* Static Checklist list of elements */}
          <div className="space-y-3 pt-2">
            {checklist.map((item) => (
              <div
                key={item.id}
                className={`flex gap-3 items-center p-3.5 rounded-xl border select-none cursor-default transition-all ${
                  item.completed
              ...
              >
                ...
                <span className={`text-xs leading-normal font-sans ${item.completed ? "line-through text-gray-400" : "font-normal"}`}>
                  {item.text}
                </span>
              </div>
            ))}
          ...
          <div className="border-t border-white/5 pt-4 space-y-2">
            <div className="flex justify-between text-[11px] font-mono uppercase text-gray-500">
              <span>Etapas Sugeridas ({completedCount}/{checklist.length})</span>
            ...
            <div className="w-full bg-[#0a0a1a] rounded h-1.5 overflow-hidden border border-white/5">
              <div
                className="bg-gradient-to-r from-purple-500 to-orange-500 h-full transition-all duration-500 opacity-85"
                style={{ width: `${progressPercent}%` }}
              ></div>
            </div>

            <div className="flex gap-2 items-center p-3 bg-purple-500/10 border border-purple-500/15 text-[11px] text-purple-300 rounded font-sans mt-2 uppercase tracking-wide cursor-default">
              <Award size={14} className="text-purple-400 shrink-0" />
              <span>Plano Pragmático - Desbloqueie todo o material com o Ebook!</span>
            </div>
          </div>
